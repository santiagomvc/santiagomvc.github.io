...
* Nonlinear preferences — EU assumes utility is linear in probabilities. Allais's paradox showed that the difference between .99 and 1.00 probability has far more psychological impact than the difference between .10 and .11, even though both are .01 gaps. Value function is also non linear.
* Status quo bias
* Endowment effect
...
* Make it so the first 5 experiments are required and must run first, the other are optional
* Lets use 2 seeds as initial experiment exploration, and 4 if we want to confirm a config is good for the experiment.
* Define better the team of agents that we will have, one per experiment
* Agents must work in parallel, and some runs can run in parallel, but all running in parallel will brick the machine. use the lead to coordinate runs
* Make sure to the agents that those configs are the initial setup, but it's likely that they will need to explore and run multiple experiments. Remembe to include the params that they can modify.
* Remember that discounting is very important, specially in positive only domains
* Remember the main goal, find differences in behavior between the two agents
* Remember to write the plan for the agent team to build in prompt_experiment at the end.
